...
class CohereReranker:
    """
    API-based reranker using Cohere's v2 rerank endpoint - same interface
    as CrossEncoderReranker (rerank(query, candidates, top_k) ->
    list[RetrievedChunk]) so RAGService._retrieve() can use either
    interchangeably.

    Not live-verified against the real Cohere API in this codebase (needs
    a real, funded API key this repo doesn't have) - covered by
    mocked-HTTP unit tests only.
    """
    ...
    def _request_with_retry(
        ...
        raise RerankerProviderError(
            f"cohere reranking request failed after {attempt + 1} attempt(s): {last_error}"
        ) from last_error
